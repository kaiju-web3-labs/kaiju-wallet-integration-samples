...
const TopBar: React.FC<TopBarProps> = ({ address, spendableBalance, totalReserve, twitterUser }) => {
  const [userName, setUserName] = useState<string>('');
  const [userImage, setUserImage] = useState<string>('');
  const [userEmail, setUserEmail] = useState<string>('');
  useEffect(() => {
    if (twitterUser) {
      setUserName(twitterUser.displayName ?? ''); 
      setUserImage(twitterUser.photoURL ?? '');
      setUserEmail(twitterUser.email ?? ''); 
    }
  }, [twitterUser]);
